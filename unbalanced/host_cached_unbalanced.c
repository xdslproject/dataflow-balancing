	...
	int n_iters = atoi(argv[2]);
        ...
        FILE * f;     
        f = fopen("../balanced/linked_dram-all_nodes.xclbin", "r");
	...
	float * out_node_3 = (float*)malloc(N * sizeof(float));
	float * in_node_3 = (float*)malloc(N * sizeof(float));
	float * out_node_4 = (float*)malloc(N * sizeof(float));
	float * in_node_4_0 = (float*)malloc(N * sizeof(float));
	float * in_node_4_1 = (float*)malloc(N * sizeof(float));
    ...
    posix_memalign(&out_node_3,4096,100*sizeof(float)); 
    posix_memalign(&out_node_4,4096,100*sizeof(float)); 
    posix_memalign(&in_node_4_0,4096,100*sizeof(float)); 
    posix_memalign(&in_node_4_1,4096,100*sizeof(float)); 
    posix_memalign(&in_node_3,4096,100*sizeof(float)); 
	...
	for(int i = 0; i < N; i++) out_node_3[i] = 0;
	for(int i = 0; i < N; i++) out_node_4[i] = 0;
	for(int i = 0; i < N; i++) in_node_4_0[i] = 0;
	for(int i = 0; i < N; i++) in_node_4_1[i] = 0;
	for(int i = 0; i < N; i++) in_node_3[i] = 0;
	...
	cl_kernel node_3_kernel = clCreateKernel(program, "node_3", &err);
	...
	cl_mem out_node_3_buf = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, N * sizeof(cl_float), out_node_3, NULL);
	cl_mem in_node_3_buf = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, N * sizeof(cl_float), in_node_3, NULL);
	cl_mem out_node_4_buf = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, N * sizeof(cl_float), out_node_4, NULL);
	cl_mem in_node_4_buf_0 = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, N * sizeof(cl_float), in_node_4_0, NULL);
	cl_mem in_node_4_buf_1 = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, N * sizeof(cl_float), in_node_4_1, NULL);

    cl_mem all_buffers[] = {in_buf, out_buf, out_node_0_buf, out_node_1_buf, out_node_2_buf, out_node_3_buf, in_node_3_buf, in_node_4_buf_0, in_node_4_buf_1};
	...
	// TODO: Using extra dedicated buffers, as the mapping between interstage buffers is wrong in the
	// bitstream. Affects the results but not the run time.
	//err  = clSetKernelArg(node_3_kernel, 0, sizeof(cl_mem), &out_node_0_buf);
	err  = clSetKernelArg(node_3_kernel, 0, sizeof(cl_mem), &in_node_3_buf);
	err  = clSetKernelArg(node_3_kernel, 1, sizeof(cl_mem), &out_node_3_buf);
	err  = clSetKernelArg(node_3_kernel, 2, sizeof(cl_int), &iters_node_3);

	err  = clSetKernelArg(node_4_kernel, 0, sizeof(cl_mem), &out_node_2_buf);
	err  = clSetKernelArg(node_4_kernel, 1, sizeof(cl_mem), &in_node_4_buf_0);
	err  = clSetKernelArg(node_4_kernel, 2, sizeof(cl_mem), &in_node_4_buf_1);
	err  = clSetKernelArg(node_4_kernel, 3, sizeof(cl_mem), &out_buf);
	err  = clSetKernelArg(node_4_kernel, 4, sizeof(cl_int), &iters_node_4);

    err = clEnqueueMigrateMemObjects(commands, 9, all_buffers, 0, 0, NULL, NULL);
    clFinish(commands);
	...
	cl_event node_3_waitlist[] = {node_0_done[0]};
	err = clEnqueueTask(commands, node_3_kernel, 1, node_3_waitlist, &node_3_done[0]);
	cl_event node_4_waitlist[] = {node_2_done[0], node_3_done[0]};
	err = clEnqueueTask(commands, node_4_kernel, 2, node_4_waitlist, &node_4_done[0]);

	if(!pipelined)
		clFinish(commands);

	for(int graph_iter = 1; graph_iter < n_iters; graph_iter++) {
		...
		cl_event node_3_waitlist[] = {node_0_done[graph_iter], node_3_done[graph_iter-1]};
		err = clEnqueueTask(commands, node_3_kernel, 2, node_3_waitlist, &node_3_done[graph_iter]);
		cl_event node_4_waitlist[] = {node_2_done[graph_iter], node_3_done[graph_iter], node_4_done[graph_iter-1]};
